...
# Note: this is a draft and used for testing, not used in the final version -> estimate.py

import pandas as pd

# set up path
path = "/home/michal/dev/sk2023/real-time/2020/"
...
# read current lists
list_overview = pd.read_csv(path + "download/list_overview.csv")
list_parties = pd.read_csv(path + "download/list_parties.csv")

# read current data
last_name1 = list_overview.loc[list_overview.index[-1], 'date']
last_name2 = list_parties.loc[list_parties.index[-1], 'date']

# ** TESTING **:
testing = pd.DataFrame(columns = parties['id'].unique().tolist())
for n in range(0, 1):
  last_name1 = list_overview.loc[list_overview.index[n], 'date']  # 4: full exist, 3: none exist
  last_name2 = list_parties.loc[list_parties.index[n], 'date']

  current_overview = pd.read_csv(path + "download/downloaded/overview/nrsr2020_med_sv_okr_" + last_name1 + ".csv", sep="|")
  current_parties = pd.read_csv(path + "download/downloaded/parties/nrsr2020_med_ps_okr_" + last_name2 + ".csv", sep="|")
  ...
  # ESTIMATIONS
  estimates = pd.DataFrame(columns=['code', 'id', 'votes'])
  ...
  r = results.copy()
  r.index = r['id']
  testing = pd.concat([testing, pd.DataFrame(r['percentage']).T], ignore_index=True)
  ...
  results
  counted = current_overview['P_OKRSOK'].sum()
  print(counted)

print(testing)

# estimate intervals
all_real_results = testing.iloc[len(testing) - 1]
important2 = all_real_results[all_real_results > 1].index.tolist()
errs = testing / all_real_results
errs_1 = 1 / errs

errx = pd.concat([errs.loc[:, important2], errs_1.loc[:, important2]], axis=1)
errx.loc[:].quantile(q=0.95, axis=1) * 1000





importants = [3, 4, 6, 11, 12, 15, 19, 24]
real_results = [6.22, 8.24, 5.77, 25.02, 6.94, 4.66, 18.29, 7.97]



ie = initial_estimates.copy()
ie.index = ie['id']

ie.T.loc[['estimate'], importants] / real_results

errors1 = abs(testing.loc[:, importants] - real_results)
errors2 = testing.loc[:, importants] / real_results



er1 = errors1.sum(axis=1)
er2a = errors2.max(axis=1)
er2b = errors2.min(axis=1)

print(er1)
print(er2a)
print(er2b)

rates = (testing.loc[0:5, importants] / real_results)
rates.loc[:, [3, 6, 11, 12]].mean(axis=1) * 1000
rates.loc[:, [19, 24]].mean(axis=1) * 1000

x = pd.pivot_table(estimates, index=['id'], columns=['code'], values=['votes'], aggfunc='sum').fillna(0).T.loc[:, importants]
x.to_csv(path + "test.csv")


# add party name
# results = results.merge(parties, on='id', how='left')
pd.options.display.float_format = '{:.2f}'.format

# parties

# Problems:
# 1: none is default (cities), but start is estimated (villages) - exactly the opposite effect
# 2: none and start: right are generally underestimated (cities) / SaS, PS, OLaNO; left are generally overestimated (villages) / SMER, LSNS
# 3: the same
# 4: problematic estimate of growing turnout in BA + other cities